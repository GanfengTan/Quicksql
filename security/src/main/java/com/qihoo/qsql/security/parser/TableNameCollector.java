package com.qihoo.qsql.security.parser;
...
public class TableNameCollector implements SqlVisitor<QueryTables> {
    ...
    public QueryTables parseTableName(String sql,FrameworkConfig config) throws SqlParseException {
    ...
    private QueryTables validateTableName(QueryTables tableNames) {
        for (String tableName : tableNames.tableNames) {
            if (tableName.split("\\.", -1).length > 2) {
                throw new RuntimeException("only support structure like dbName.tableName,"
